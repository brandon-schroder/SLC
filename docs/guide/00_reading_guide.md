...
>
> Written at commit `d7b7b27` (2026-07-07), tracker refreshed at `96e9d4a`
> (2026-07-08); both lint gates green, milestones M0–M8 all closed (plus the
> post-M8 Tier-3 stabilization, colored-FD Jacobian, and ongoing Lieblein
> loss calibration; Guide 3 §4). Guide numbers are regenerated by the scripts
> in [`_traces/`](_traces/) — see §4 and [`_traces/README.md`](_traces/README.md).
...
- **Numbers shown are real, and are snapshots of a moving target.** Where a
  guide quotes iteration counts, norms, or performance values, they were
  produced by running the code at the header commit, not estimated. This
  solver is under active development (loss correlations being calibrated,
  drivers tuned), so **performance numbers drift** — the *mechanics* the
  guides explain are stable, the *magnitudes* are not. Volatile numbers are
  therefore quarantined into stamped **snapshot blocks** whose generator is a
  script in [`_traces/`](_traces/) (see [`_traces/README.md`](_traces/README.md)).
  Refreshing a stale number is `python docs/guide/_traces/trace_guideN.py` →
  paste → bump the stamp, not a re-derivation. A `validity: 0` on a converged
  run is the signal that a case has drifted out of its correlations'
  calibration range and its performance numbers should not be trusted as
  clean measurements.
