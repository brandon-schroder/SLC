...
>
> Written at commit `d7b7b27` (2026-07-07); §8's closing check verified at
> `96e9d4a` (2026-07-08) — it still lands on run A's PR exactly. The fixed
> illustrative B.2/B.3/B.4 conversions do not move with calibration; only
> run A's actual Δs does. All of §8's numbers are regenerated by
> [`_traces/trace_guide2.py`](_traces/trace_guide2.py) (snapshot convention:
> [`_traces/README.md`](_traces/README.md)).
