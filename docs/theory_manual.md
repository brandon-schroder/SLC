...
**Row-throat capacity check (implemented 2026-07).** For every row whose §4.1 geometry provides a throat opening $o(y)$, the classical driver evaluates the 1-D blade-passage sonic capacity $\dot m_{th} = c_d\,Z \sum_k \rho^*_k a^*_k\, o_k\, \Delta q_k$ on the mid-passage q-o, with the sonic state on the local isentrope of the **relative** stagnation enthalpy from rothalpy re-referencing (Appendix B.1) at $r_{th} = (r_{LE}+r_{TE})/2$; a converged solution with $\dot m > \dot m_{th}$ is declared CHOKE_LIMITED post-solve (the solved-state-check pattern; `row_throat_capacity`, drivers). Measured on the TN D-6967 two-stage turbine this lands the machine choke within ~1% of the rig's (rotor-1 throat binds just above the annulus limit); on the supersonic-inlet Rotor 37 the gauging throat correctly does **not** bind (a started supersonic rotor chokes at its inlet swallowing limit). The check is a lower-bound declaration, not a flow-field constraint — flow-responding throat physics (in-passage stations) remains the recorded extension, as does wiring $c_{row}$ into the §6.7 continuation margins and the Newton driver.
...
**Structural gate** (as V5/V6), and the **first radial end-to-end**: a backswept centrifugal compressor impeller composed through the `Machine` facade with the centrifugal set (`slcflow/verification/v7_centrifugal.py`) — Wiesner slip + internal loss (incidence + skin-friction + the dominant blade-loading diffusion loss) — on the parametric $\phi:0\to90°$ axial→radial path (M1), **doing** real centrifugal work ($\Delta h_0>0$, PR $>1$) with real loss ($\Delta s>0$), spinning axial inflow ($rV_\theta=0$) up to a slipped exit swirl ($V_{\theta,2}<U_2$), and exiting radially ($r_{\text{exit}}=r_2$, i.e. $\phi=90°$). Reference geometry is a representative impeller (concentric 90°-bend walls, $r_2=0.25$ m, bend radii $0.08/0.18$ m, $\Omega=1450$ s⁻¹ → $U_2=362$ m/s, 18 blades, 30° backsweep), **not** a digitised Eckardt rotor. Mass flow re-centred $\dot m = 12 \to 17$ kg/s at the 2026-07 blade-loading landing (see the operating-point note below).

| Quantity | Tier 1 ($N_{sl}=1$) | Tier 2 ($N_{sl}=7$) | Tier 3 ($N_{sl}=7$) | Band / check |
|---|---|---|---|---|
| PR (total-to-total) | 2.097 | 2.027 | — (fold) | $(1.5, 4.0)$, $>1$ (compression) |
| efficiency | 0.839 | 0.828 | — (fold) | $(0.6, 0.999)$ |
| exit $rV_\theta$ (from inlet 0) | 57.8 | — | — | $>0$ (spun up) |
| exit $V_\theta / U_2$ (slip) | 0.638 | — | — | $\in(0,1)$ (Wiesner $\sigma<1$) |
| $r_{\text{exit}}$ [m] | 0.25 | 0.25 | — | $=r_2$ ($\phi=90°$) |
| validity | 1.000 | 1.000 | — | $>0.5$ |

Tier 1 and Tier 2 agree on PR to $<4\%$. **Efficiency reads ~0.83–0.84** now that the dominant blade-loading loss is modelled with the corrected Coppage/Oh-1997 diffusion factor (it was ~0.98 with only incidence + skin-friction, and read ~0.80 under the earlier inverted `W1/W2` loading term that over-estimated the loss ~2.3×; the ratio was fixed 2026-07-12 — see CENT-LOSS.md); tip-clearance/disk-friction stay deferred. Point-by-point Eckardt reproduction and stage-map traversal are **[VERIFY]**, blocked on the reference library and the deferred loss components.

**Operating-point re-centre + the two-disease wedge diagnosis (2026-07).** The dominant blade-loading (diffusion) loss (Coppage/Aungier 5.15, ~7 kJ/kg, CENT-LOSS.md — the physics that brings $\eta$ to a realistic ~0.80) initially broke *both* spanwise tiers, and was landed with both as `xfail` wedge tripwires. The 2026-07 characterization (`probe_cin_*`, memory `wedge-closure-in-newton`) **separated the single "wedge" into two distinct diseases and cracked one:**

- **Tier 2 = an operating-point stratification-capacity fold (CRACKED).** The realistic stratified exit loss drives an interior streamtube's meridional $V_m$ toward the master-ODE $V_m=0$ singularity, so *below* a mass-flow floor (~15 kg/s) the **coupled** spanwise flow folds — even though each station's A.7 capacity individually stays $\gg\dot m$ (the fold is coupled, not per-station). It is **not** closure coupling (prescribing the stratified transport *fixed* folds identically) and **not** the classical repositioning algorithm (a global Newton on the flow folds too); closure-in-Newton was measured **not** to help, and the fold terminates *at* the $V_m=0$ singularity so continuation/pseudo-arclength has no far side to reach. Raising $\dot m$ lifts every $V_m$ off the singularity: the feasible window is $\dot m\in[\sim16,\sim20]$ (fold below, genuine choke above). At the re-centred $\dot m=17$ Tier 2 converges with realistic loss (validity 1, PR 1.969, $\eta$ 0.803) — a **passing** structural test (`test_tier2_converges_with_realistic_loss`). Same category of case-design re-centre as the V5 validity-0 annulus retune and the transonic-V5 $\beta_2$ retune.

- **Tier 3 = a physical feasibility fold at realistic loss (STILL OPEN, but NOT a solver gap).** Raising $\dot m$ does **not** fix Tier 3: it fails at *every* non-choked mass flow. The 2026-07 diagnosis (`probe_v7t3_*`, a damped-Newton + curvature-strength continuation on the flow at fixed converged closures) showed the flow branch **folds** — an interior streamtube's $V_m\to0$ (the master-ODE singularity) — at only $\sim9\%$ of the full Tier-3 curvature at $\dot m=17$ ($\sim26\%$ at $\dot m=20$). The tight 0.08 m bend ($\kappa\sim20$) makes the curvature term $\text{curvature}\cdot(\kappa\cos\varepsilon)\cdot V_m$ swing the spanwise $V_m$ enormously (measured $+200$ to $+477$ m/s), and with the realistic-loss stratification it drives an interior streamtube to $V_m\to0$ (incipient meridional reversal the inviscid model refuses) before full curvature. The fold is $\dot m$-liftable ($c_{\text{fold}}=0.086\to0.263$ as $\dot m=17\to20$, the same mechanism as the Tier-2 wedge) but reaching full curvature ($c=1$) extrapolates to $\dot m\approx32\gg$ the Tier-2 choke ceiling $\sim22$ — so no operating point admits full radial equilibrium. **There is no positive-$V_m$ root**, so this is **not** the "robust radial/mixed repositioning" item and a stiff integrator / compact-support fit / damped Newton cannot help (the "repositioning failed" symptom is downstream of the fold). The case-side levers are a calibrated/lower blade-loading loss (`[VERIFY]`, likely high), a gentler bend, or beyond-model-validity acceptance. **The calibrated-loss lever was tried (2026-07-12): the Coppage/Oh-1997 $D_f$ ratio fix cut the blade-loading loss $\sim2.3\times$ (CENT-LOSS.md); it EASED the fold — Tier 3 now fails at sane PR/η ($\sim2.3/0.9$) rather than garbage, and the milder V8 55° bend was lifted into a converging window by it — but did NOT crack V7's tight 90° bend: Tier 3 still fails at every $\dot m$ in 13…32.** Stays an `xfail` tripwire (`test_tier3_infeasible_fold_at_realistic_loss`). Memory `v7-tier3-root-cause`.

(History, M7-4: originally "Tier-3 radial repositioning needs in-blade subdivision"; the 2026-07 stabilization refuted that for the *pre*-blade-loading case — edge-only converges post-stabilization, 173 iterations, PR 2.4433 vs. subdivided 2.4540 — a driver-artifact, not a repositioning envelope. INBLADE stays the §2.5/§3.4 *resolution* choice. That revision stands; the realistic-loss Tier-3 collapse above is a separate matter.)

### C.8 V8 — Mixed-flow compressor, structural (bound at M8-4; `tests/test_v8_mixed_flow.py`)

**Structural gate at all three tiers (Tier 3 since the 2026-07 stabilization; re-centred to $\dot m=14$ at the 2026-07-12 blade-loading fix)** — a mixed-flow impeller (`V8MixedFlow`) composed through the `Machine` facade with the centrifugal set on a **partial** axial→radial bend (exit $\phi_{max}=55°$, bend centre at $r_c=0.25$ m, $\Omega=1450$ s⁻¹, $\dot m=14$ kg/s). Converges at every tier, compresses ($\eta\approx0.87$ with the realistic blade-loading loss; Tier-1 PR $=1.451$, Tier 2 $1.449$, Tier 3 $1.413$) with real loss, spins axial inflow up to slipped exit swirl ($V_\theta/U_2\approx0.45$), and exits mixed-flow: $r_{LE}=0.131 < r_{exit}=0.178 < r_c=0.25$, i.e. the flow turned partway ($0<\phi<90$) with a radius rise — neither the axial V5 nor the fully-radial V7.

**Measured (M8-4): "Tier-3 mixed-flow repositioning is beyond the current stabilization" — REVISED 2026-07: diagnosed and fixed; the attribution was wrong.** The failures across the $(n_{sl}, n_{inblade}, \Omega)$ grid were **not** the §6.4 odd-even repositioning mode (measured: streamlines barely move before death, pure repositioning-with-curvature on the same bend is stable, and station/spanwise density is irrelevant). The actual chain: (i) the master ODE's RHS $\sim$ core$/V_m$ is singular at $V_m=0$; integrating a q-o from boundary values stale relative to the transported fields — the unrelaxed closure switch-on being the main producer, amplified by the REE swirl term $\sim rV_\theta/r^2$ at low-radius mid-bend stations — crosses that singularity into non-finite or negative-$V_m$ garbage; (ii) the driver *fatally* boundary-checked the stale-guess split that the per-q-o continuity solves were about to repair (the killed states were proven solvable: cold re-solve max$|F|/\dot m = 4\times10^{-15}$); (iii) `_solve_qo` could accept roots on spurious negative-$V_m$ branches or at the out-of-domain cliff. The V7 "pocket" and the V8 "angle-specificity" were chaos in whether the transient garbage stayed finite — not a stability envelope. **Stabilization (2026-07):** the AD-10 flow-field check moved to the solved state (a broken *metrics* fit stays fatal); continuity roots are validated onto the strictly-positive branch; a q-o transiently without a positive-branch root has its boundary value frozen, with CHOKE_LIMITED declared only after `choke_patience` consecutive deficient iterations; and the *first* closure application is relaxed from the duct baseline through the same §6.2.4 rule as every later one. With these, V8 Tier 3 **converges** (396 iterations at the §6.4 throttle $\omega_{sl}\approx0.066$ — slow; acceleration, e.g. Newton finishing, is the recorded follow-up) to PR 1.587, within a few percent of Tier 2. Bands are structural plausibility gates; every centrifugal coefficient stays `[VERIFY]`. **Updated 2026-07 by the blade-loading loss, then 2026-07-12 by the D_f ratio fix + re-centre.** The dominant blade-loading (diffusion) loss (Coppage et al. 1956 / Oh 1997, CENT-LOSS.md) lifts $\eta$ to a realistic level; it initially pushed V8 **at Tier 3** into a narrow choke/max-iter pocket (choke_limited at the old $\dot m=12$; a knife-edge $\dot m\approx15$, 593 iterations), too narrow to pin robustly, so V8 was held at $\dot m=12$ with Tier 3 an `xfail`. **The 2026-07-12 Coppage/Oh-1997 $D_f$ loading-term ratio fix** (`W1/W2`→`W2/W1`, ~2.3× less blade-loading loss → ~27–30% less spanwise stratification; CENT-LOSS.md) **LOWERED that pocket into a genuine converging window $\dot m\in\{13,14\}$** (choke at 12, slow-max-iter at 15/16 — measured sweep). Re-centring to $\dot m=14$ (the case default now) makes Tier 3 a **passing** test at all three tiers (validity 1; Tier-3 PR 1.413 agrees Tier 2 1.449 to $\sim2.5\%$) — the same operating-point re-centre as V7 Tier 2 ($12\to17$). It is still **slow** at the §6.4 throttle $\omega_{sl}\approx0.066$ (Newton finishing / §6.4 recalibration remain the recorded acceleration follow-ups); the pocket is real but not wide. Unlike V7's tighter 90° bend (a hard infeasible fold at *every* mdot even with the reduced loss, C.7), V8's milder 55° bend IS liftable this way. `test_tier3_converges_at_recentred_mdot`. Memory `wedge-closure-in-newton`, `centrifugal-blade-loading-wip`. **§6.4-recalibration follow-up DELIVERED (2026-07-19): the per-case `wilkinson_c` override.** C.3 measured $c^*=13.2$ safe for exactly this layout ($\phi=55°$, $n_{inblade}=6$; identical answers, 17.6 fails), so the V8 case now carries `wilkinson_c=13`: Tier 3 converges in **~153 outer iterations (2.6× faster than 395**, inside the stock `max_outer=200`) and the **pocket WIDENS from $\{13,14\}$ to $\{13,14,15\}$** ($\dot m=15$ lifts from slow-max-iter into a 264-iteration convergence). The boundaries are unmoved by relaxation speed — $\dot m=12$ stays the capacity/stratification CHOKE fold, $\dot m\ge16$ the upper feasibility edge (MAX_ITER at 800 even at $c=13$) — so Newton-finishing / closure-in-Newton stays the remaining lever for the pocket *edges*, while the interior is now fast and a step wider. `test_tier3_pocket_widened_by_wilkinson_c` pins the widening (with a `wilkinson_c=4.4` control that fails).
