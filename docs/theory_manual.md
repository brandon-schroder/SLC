...
**Measured (M7-4): Tier-3 radial repositioning needs in-blade subdivision.** Unlike the axial V5/V6 (straight annulus → Tier 3 ≡ Tier 2 bit-for-bit, C.4), V7 is the first curved path carrying a blade row *and* streamline repositioning. With edge-only stations the §6.4 odd-even streamwise mode diverges at any relaxation, and the M5 Newton driver inherits the same stiff seed and cannot recover it; **six INBLADE stations** (`n_inblade=6`, M7-3) keep the per-step curvature inside the envelope and converge all tiers — the concrete physical reason radial rows want in-blade stations. The stable pocket is narrow (odd counts near the edge still flake); a robust radial-repositioning stabilization is a carryover past M7. The edge-only failure is pinned in-suite as a tripwire (`test_v7_centrifugal.py::test_tier3_edge_only_is_the_measured_inblade_necessity`, bound at the 2026-07 audit follow-up, mirroring C.8's), so the necessity claim stays executable rather than narrative.

### C.8 V8 — Mixed-flow compressor, structural (bound at M8-4; `tests/test_v8_mixed_flow.py`)

**Structural gate at Tier 1 + Tier 2** — a mixed-flow impeller (`V8MixedFlow`) composed through the `Machine` facade with the centrifugal set on a **partial** axial→radial bend (exit $\phi_{max}=55°$, bend centre at $r_c=0.25$ m, $\Omega=1450$ s⁻¹, $\dot m=12$ kg/s). Converges at Tier 1 (meanline) and Tier 2 (REE), compresses (PR $=1.56$, $\eta\approx0.96$) with real loss, spins axial inflow up to slipped exit swirl ($V_\theta/U_2\approx0.45$), and exits mixed-flow: $r_{LE}=0.131 < r_{exit}=0.178 < r_c=0.25$, i.e. the flow turned partway ($0<\phi<90$) with a radius rise — neither the axial V5 nor the fully-radial V7.

**Measured (M8-4): Tier-3 mixed-flow repositioning is beyond the current stabilization.** The V7 90°-bend Tier-3 pocket (`n_sl=7, n_inblade=6`, C.7) **does not transfer** to intermediate angles: across a wide $(n_{sl}, n_{inblade}, \Omega)$ grid at $\phi_{max}\in[45°,70°]$, Tier-3 full-SLC repositioning fails to converge (the §6.4 odd-even mode again, but the working pocket is not just narrow — it is *angle-specific*). So V8 is structural at Tiers 1–2 only; Tier 3 is the **robust radial/mixed repositioning stabilization** carried past M7, a driver-stabilization gap (not a closure or geometry one). `test_tier3_is_the_known_repositioning_carryover` pins this as a tripwire — it fails loudly the day a stabilization makes Tier 3 converge. Bands are structural plausibility gates; every centrifugal coefficient stays `[VERIFY]`, and $\eta$ reads high (only incidence + skin friction modelled) as for V7.
