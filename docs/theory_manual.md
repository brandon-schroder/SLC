...
- **Tier 3 = a separate curvature-repositioning collapse (STILL OPEN).** Raising $\dot m$ does **not** fix Tier 3: it fails at *every* mass flow (including the Tier-2-feasible window), dying early (outer iteration 3–5) with the §6.4 `wilkinson_c` throttle inert. This is the Tier-3 curvature + lean repositioning feedback on the 90° bend — the standing **robust radial/mixed repositioning** open item, now cleanly isolated from the operating-point confound. It stays an `xfail` tripwire (`test_tier3_hits_the_repositioning_collapse`); the recorded attacks are a compact-support / end-condition-aware streamline fit or closure-in-repositioning (both major, not patches). The reposition-freeze / capacity-peak robustness patches were re-measured non-curative and not shipped. Memory `wedge-closure-in-newton`, `centrifugal-blade-loading-wip`.
