...
**Measured (M7-4): "Tier-3 radial repositioning needs in-blade subdivision" — REVISED 2026-07: refuted.** Unlike the axial V5/V6 (straight annulus → Tier 3 ≡ Tier 2 bit-for-bit, C.4), V7 is the first curved path carrying a blade row *and* streamline repositioning. M7-4 originally recorded that edge-only stations diverge "the §6.4 odd-even streamwise mode at any relaxation" and that six INBLADE stations were measured-necessary (a narrow pocket). The 2026-07 stabilization diagnosis refuted the attribution: the edge-only failure was the classical driver accepting a continuity root on a **spurious negative-Vm branch** (mass balancing by sign cancellation → decreasing mass cumulative → abort), one member of the driver-artifact family described in the C.8 revision — not a repositioning envelope. Post-stabilization the edge-only case converges (173 iterations) to the same answer as the subdivided one (PR 2.4433 vs. 2.4540, <0.5%). INBLADE stations remain the *resolution* choice for in-blade work/loss distributions (§2.5, §3.4) — they are no longer a convergence requirement. **Superseded 2026-07 by the blade-loading loss.** Adding the dominant blade-loading (diffusion) loss (Coppage/Aungier 5.15, ~7 kJ/kg, CENT-LOSS.md) — the correct physics that brings $\eta$ to a realistic ~0.90 — drives V7's 90° bend into the **freeze-fallback wedge at BOTH Tier 2 and Tier 3** (the exit q-o loses its positive-branch continuity root; lowering $\dot m$ makes it worse; a Tier-2 retune to $\dot m=10$ still only slow-max-iters). The Tier-3 stabilization above holds for lower-loss cases but is overwhelmed by realistic centrifugal loss on the 90° bend; the reposition-freeze / capacity-peak robustness patches were re-measured (non-curative, changed no outcome) and not shipped. V7 is therefore **meanline-only** with realistic loss; the spanwise tiers are pinned as `xfail` wedge tripwires (`test_tier2/tier3_hits_the_wedge_with_realistic_loss`), flip when the wedge is cracked (closure-in-Newton / compact-support fit — the #1 open item). Memory `centrifugal-blade-loading-wip`.
