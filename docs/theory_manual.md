...
| Configuration ($N_{sl}=9$, Tier 3, validity 1.0) | Converged | PR (t-t) | exit $\Delta s$ span spread |
|---|---|---|---|
| **mixing off** | yes | 1.121 | 0.267 J/(kg·K) |
| **mixing on** (default Gallimore, $c_{mix}=5\times10^{-4}$) | yes | 1.121 | 0.218 J/(kg·K) |

**Revised three times in 2026-07 — a cautionary entry.** M8-3 originally recorded the un-mixed two-stage as a NUMERICAL_FAILURE with a runaway ($\sim$40 J/(kg·K)) entropy split and concluded mixing was a *convergence prerequisite*; the Tier-3 stabilization (see C.8) showed that was the driver's stale-split / spurious-branch artifact, and the un-mixed case converges cleanly. The revision then recorded a *dramatic* difference ($\sim$25×, 17.6 vs. 0.69 J/(kg·K)) — the compound artifact of the Lieblein $\bar\omega$-inversion (~4× too much loss) and a $c_{mix}=0.01$ that was ($\sim$20×) stronger than G–C's calibration (GC86.md). With **both** fixed, and the case geometry retuned so the loss runs **in** the Lieblein validity window (the earlier configs ran at closure validity 0 — over-loaded untwisted blades driving $D_{eq}$ out of window, so the loss was saturated), the measured effect is the modest $\sim$18% above, holding at $\sim$14% at three and four stages (2/3/4 stages: 0.27/2.73/5.23 J/(kg·K) un-mixed) while the absolute spread grows — i.e. mixing does **not** catch up with the stratification production. The direction (mixing reduces the spread) is a guaranteed operator property; the *smallness* is the finding the regression now pins. The "mixing flattens multistage stratification" narrative does not survive an honestly-calibrated coefficient on in-window loss. Bands are structural plausibility gates as for single-stage V5; $c_{mix}$ is G–C-calibrated (option B, r-based retune; GC86.md).
